...
let make_wrappers ast types (linenum_maps: ((int * string) option) array)
  ...
  Hashtbl.iter (fun tname (def:hol_def) ->
      ...
      (* There could be an extra argument given to it compared to the number of
        args in the type. It can be:
        - (assumptions,goal) part for tactic
        - the input term for conv
        For tactic, strip away the extra arguement. For conv, keep the extra
        argument because it is the term that conv tries to evaluate, which
        is absolutely a very useful information. *)
      let numargs,arg_defs,argtys =
        let _ =
          if List.length def.arg_defs > numargs + 1 then
        ...
        match def.kind with
        | KindTactic ->
          (numargs,(takelist numargs def.arg_defs),argtys)
        | KindConversion ->
          ((List.length def.arg_defs),def.arg_defs,
            (argtys @ [(TyConst "term"),None,false])) in
